...
Digest= module.exports= function (options) {
  ...
  that.authenticate= function(req, res, callback) {
    ...
    if(isDigest) {
      ...
      var href= req.url
      my._getPasswordForUser(credentials.username, function(error, password, details){
        if(error) callback(error);
          ...
          if( password !== undefined && myResponse == credentials.response ) {
            self.success({ username : credentials.username, details: details}, callback);
          }
          else {
